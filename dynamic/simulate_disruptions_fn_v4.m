...
% v4: Also creating outputs for prevalence
...
for ii = 1:size(xs,1)
    ...
    prev(:,ii,1)   = sum(soln0(:,s.prevalent),2);
...
%     notq2(:,:,ii,1) = diff(soln0(1:3:end,i.aux.noti(1)),1);
    ...
    prev(:,ii,2)   = sum(soln1(:,s.prevalent),2);
...
% Take annual average of prevalence
dims = size(prev);
prev(1,:,:) = [];

tmp  = reshape(prev, [12, 6, dims(2:end)]);
prev_annu = squeeze(mean(tmp,1))*1e5;

prev = [repmat(prev_hist,[1,1,2])*1e5; prev_annu];

tmp = prctile(prev,[2.5,50,97.5],2);
prev_mid = squeeze(tmp(:,2,:));
