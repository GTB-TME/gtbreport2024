...
f5.1.1a_data <-
  ...
  mutate(who_reg=factor(recode(who_reg,`953`="African Region",
...
f5.1.1a_txt <- f5.1.1a_data %>%
  filter(who_reg == "GLOBAL", year == 2000|year == 2019|year == report_year-3) %>%
  pivot_wider(names_from = "year", values_from = "SCI") %>%
  rename(sci_2021 = `2021`, sci_2019 = `2019`, sci_2000 = `2000`)

f5.1.1a_txt <- f5.1.1a_data %>%
  filter((who_reg == "Region of the Americas"|who_reg == "African Region"|who_reg == "European Region")&year == report_year-3) %>%
...
f5.1.1b_data <- f5.1.1b_data %>%
  mutate(income_group=factor(recode(income_group,`910`="High-income",
