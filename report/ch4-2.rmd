...
As of `r report_year`, funding data (both commitments and disbursements) were provided by 36 multilateral donor organizations, 32 members of the OECD’s DAC (31 individual countries and the European Union) and a further 18 countries beyond the DAC that report to the OECD. Disbursement data include both direct transfers to countries and the provision of goods and services (e.g.&nbsp;in-kind transfers or technical assistance). 
...
`r lnk("Fig. 4.2.1")` shows trends in international donor funding for developing countries (as defined by the OECD) between 2013 and `r latest_year`, both overall and for each WHO region. The total funding from all sources in `r latest_year` was US\$&nbsp;`r all_donation` million, with a large increase (+`r ftb( (all_donation / (filter(Fig1, donor=="Total", year==latest_year-1) %>% slice(1) %>% select(value) %>% unlist()) -1 )*100 )`%) from US\$&nbsp;`r ftb(filter(Fig1, donor=="Total", year==latest_year-1) %>% slice(1) %>% select(value) %>% unlist())` million in 2021.

### `r anch("Fig. 4.2.1")` <span class="fig" style="color:#F21905">Fig. 4.2.1</span> International donor funding for TB  by source based on donor reports to the OECD, 2013&ndash;`r latest_year`
...
<div class="footnote">OECD: Organisation for Economic Co-operation and Development; WHO: World Health Organization.<br>^a^ The OECD defines developing countries and areas as those eligible to receive ODA. The group currently includes `r Fig1_txt` countries and areas, of which `r Fig1_lmic_txt` are LMICs. The subtotals for the six WHO regions do not sum to the total for all low and middle-income countries combined. This is because funding recorded in regional (rather than country-specific) categories used by the OECD cannot be matched to a specific WHO region, and the recipient country for country-level funding is sometimes not specified.</div>

<hr>
<br />
From 2013 to `r latest_year`, The Global Fund was consistently the largest provider of international donor funding in LMICs, with its share averaging `r ftb(gf_contribution)`% in this period. In `r latest_year`, the total was US\$&nbsp;`r gf_donation` million (`r round(100*gf_donation/all_donation,0)`% of the global total of US$&nbsp;`r all_donation` million). 

Annual disbursements of bilateral funding from the United States of America (US) between 2013 and `r latest_year` that were recorded in the OECD database (`r lnk("Fig. 4.2.1")`) ranged from US\$&nbsp;`r ftb(Fig1 %>% filter(donor == "United States of America" & g_whoregion =="Developing countriesᵃ" & year==2016) %>% select(value) %>% unlist())` million to US\$&nbsp;`r ftb(Fig1 %>% filter(donor == "United States of America" & g_whoregion =="Developing countriesᵃ" & year==2018) %>% select(value) %>% unlist())` million; in `r latest_year`, the total amount disbursed was US\$&nbsp;`r ftb(Fig1 %>% filter(donor == "United States of America" & g_whoregion =="Developing countriesᵃ" & year==latest_year) %>% select(value) %>% unlist())` million. These amounts are lower than official US congressional allocations for TB (`r ref_lnk("5, 6")`), which have grown from US\$&nbsp;`r ftb(Fig2 %>% filter(year == 2013) %>% select(value) %>% unlist())` million in 2013 to US\$&nbsp;`r ftb(Fig2 %>% filter(year == latest_year-1) %>% select(value) %>% unlist())` million in `r latest_year-1`, US\$&nbsp;`r ftb(Fig2 %>% filter(year == latest_year) %>% select(value) %>% unlist())` million in `r latest_year` and US\$&nbsp;`r ftb(Fig2 %>% filter(year == report_year) %>% select(value) %>% unlist())` million in `r report_year-1` and `r report_year` (`r lnk("Fig. 4.2.2")`).   


### `r anch("Fig. 4.2.2")` <span class="fig" style="color:#F21905">Fig. 4.2.2</span> Official US congressional allocations of funding for TB, 2013&ndash;`r report_year`
...
When bilateral funding from individual countries reported to the OECD is combined with estimates of individual country contributions for TB that are channelled through The Global Fund, the US government is by far the largest source of international donor funding for TB (`r lnk("Fig. 4.2.3")`). In the period 2013&ndash;`r latest_year`, it provided `r ftb(Fig3 %>% filter(donor == "United States of America") %>% select(pcnt) %>% unlist())`% of the total amount of donor funding for TB from entities that report to the OECD; in the most recent year for which data are available (`r latest_year`), the share was `r ftb(us_trend %>% filter(year == latest_year) %>% select(us_pcnt) %>% unlist())`%. The next largest individual country contributors in the period 2013&ndash;`r latest_year` were (in order of their contribution) the United Kingdom of Great Britain and Northern Ireland (`r ftb(Fig3 %>% filter(donor == "United Kingdom") %>% select(pcnt) %>% unlist())`%), France (`r ftb(Fig3 %>% filter(donor == "France") %>% select(pcnt) %>% unlist())`%), Germany (`r ftb(Fig3 %>% filter(donor == "Germany") %>% select(pcnt) %>% unlist())`%), Canada (`r ftb(Fig3 %>% filter(donor == "Canada") %>% select(pcnt) %>% unlist())`%) and Japan (`r ftb(Fig3 %>% filter(donor == "Japan") %>% select(pcnt) %>% unlist())`%).  



### `r anch("Fig. 4.2.3")` <span class="fig" style="color:#F21905">Fig. 4.2.3</span> Distribution of international donor funding for TB when Global Fund contributions are allocated to individual countries and other reporting entities,^a^ 2013&ndash;`r latest_year`
...
``` {r fig_4.2.3 , fig.alt="International donor funding (in US$ millions) for TB prevention, diagnosis and treatment from individual countries", fig.height = 8, , fig.width = 12}

Fig4_2_3_plot <- ggplot2::ggplot(Fig3,aes(area=pcnt,fill=donor,label=donor))+ 
...
output_ggplot(Fig4_2_3_plot, Fig3, show_static = T, pdf_csv_folder, save_csv, save_pdf)


```
<div class="footnote">United Kingdom: United Kingdom of Great Britain and Northern Ireland; WHO: World Health Organization.<br>
...
`r lnk("Fig. 4.2.4")` shows that international funding for TB (US\$&nbsp;`r 1000 * (Fig4 %>% filter(year == latest_year & sector == "TB control") %>% select(value) %>% unlist() %>% round(3))` million in `r latest_year`) is about half of that for malaria (US\$&nbsp;`r (Fig4 %>% filter(year == latest_year & sector == "Malaria control") %>% select(value) %>% unlist() %>% round(1))` billion in `r latest_year`) and about 13% of that for HIV (US\$&nbsp;`r (Fig4 %>% filter(year == latest_year & sector == "STD control including HIV") %>% select(value) %>% unlist() %>% round(1))` billion in `r latest_year`). Overall, TB funding constitutes `r ftb(tb_pct$pct)`% of total ODA funding for health and population programmes (US$&nbsp;`r (Fig4 %>% filter(year == latest_year & sector == "Total health and population programmes") %>% select(value) %>% unlist() %>% round(0))` billion in `r latest_year`).



### `r anch("Fig. 4.2.4")` <span class="fig" style="color:#F21905">Fig. 4.2.4</span> International donor funding for Health and Population programmes, TB, HIV and malaria, 2013&ndash;`r latest_year`
...
4.	OECD Data Explorer [website]. Paris: Organisation for Economic Co-operation and Development; 2024 (https://data-explorer.oecd.org/).

5.	The US government and global tuberculosis efforts. San Francisco: Kaiser Family Foundation; 2024 (https://www.kff.org/global-health-policy/fact-sheet/the-u-s-government-and-global-tuberculosis-efforts/?msclkid=2a614d82caf311ecb251baaa5c42217b).

6.	US foreign assistance dashboard [website]. Washington, DC: Office of Management and Budget, Agency Congressional Budget Justifications, Congressional Appropriation Bills; 2024 (https://www.foreignassistance.gov/).
...
var fig_OECD4_data = `r Fig4 %>% pivot_wider(names_from = sector, values_from = value) %>% rename(malaria=2,tb=3,std=4,total=5) %>% toJSON("rows") `; 
...
function createFig_OECD1(fig_ID,data,filter) {
      ...
      title: {
        ...
        tooltip: {
				visible: true,
				template: "Development assistance, total (#= category #): #= num_spacer(value) # million (constant 2022 US$)"
			}
			},{
        name: "Global Fund",
				field: "gf",
        ...
        tooltip: {
				visible: true,
				template: "Global Fund (#= category #): #= num_spacer(value) # million (constant 2022 US$)"
			}
			},{
        ...
        tooltip: {
				visible: true,
				template: "United States of America (#= category #): #= num_spacer(value) # million (constant 2022 US$)"
			}
			},{
        name: "Other",
				field: "oth",
        ...
        tooltip: {
				visible: true,
				template: "Other (#= category #): #= num_spacer(value) # million (constant 2022 US$)"
			}
			},],
			valueAxis: {
				labels: {
...
function createFig_OECD4(fig_ID,data) {
      ...
      dataSource: data,			
        ...
        tooltip: {
				visible: true,
				template: "TB control (#= category #): #= value.toPrecision(2) # billion (constant 2022 US$)"
			}
			},{
        ...
        tooltip: {
				visible: true,
				template: "STD control including HIV (#= category #): #= value.toPrecision(2) # billion (constant 2022 US$)"
			}
			},{
        ...
        tooltip: {
				visible: true,
				template: "Malaria control (#= category #): #= value.toPrecision(2) # billion (constant 2022 US$)"
			}
			},{
        ...
        tooltip: {
				visible: true,
				template: "Total health and population programmes (#= category #): #= value.toPrecision(2) # billion (constant 2022 US$)"
			}
			},],
			valueAxis: {
				labels: {
