...
if(show_estimates){
  ...
  # A. Countries
  # - - - - - - - -
  coverage_inc_country <- filter(est_country, year>=2010) %>%
    select(year,
           iso3,
           e_inc_num = inc.num,
           e_inc_num_lo = inc.lo.num,
    ...
    inner_join(hbc30, by = c("iso3"))

  f2.3.3_data <- filter(notification, year>=2010) %>%
    select(entity = country,
           iso3,
           year,
           c_newinc)  %>%
    inner_join(coverage_inc_country, by = c("iso3", "year")) %>%
    select(-iso3) %>%
...
# 
  ...
  # Add EUR and EMR
  f2.3.3_txt <- filter(coverage_region_latest, entity %in% c("European Region", "Eastern Mediterranean Region", "Region of the Americas","Western Pacific Region","South-East Asia Region")) %>%
    select(entity, c_cdr) %>%
    ...
    select(c_cdr_EUR = `European Region`,
           ...
           c_cdr_SEA = `South-East Asia Region`,
    )

  # Add Mozambique pct pulmonary bac confirmed
  f2.3.3_txt <- filter(notification, year==report_year-1 & (iso3 == "MOZ"|iso3 == "PNG"|iso3 == "PHL")) %>%
    select(iso3,new_labconf, new_clindx,
           ret_rel_labconf, ret_rel_clindx) %>%
    mutate(bacconf_pct = (new_labconf + ret_rel_labconf) * 100 / (new_labconf + new_clindx + ret_rel_labconf + ret_rel_clindx)) %>%
    select(iso3,bacconf_pct) %>%
    pivot_wider(names_from = iso3,
                names_prefix = "bacconf_pct_",
                values_from = bacconf_pct) %>%
    cbind(f2.3.3_txt)


  # Create ist of countries with cdr < 50% for the text
  f2.3.3_txt_list_hi <- filter(coverage_country, c_cdr >= 80#, entity != "Mozambique"&entity != "Papua New Guinea"&entity != "Philippines"
                               ) %>% # hi TC in MOZ may be due to over reliance on clinical dx
    select(entity) %>%
    arrange(entity)
  ...
  EUR_no_data <- notification %>%
    filter(year==report_year - 1 & g_whoregion == "EUR" & is.na(c_newinc)) %>% nrow()
...
# global data for 2.3.6 top panel
if(show_estimates){
  ...
  f2.3.6_region_data <- filter(TBHIV_for_aggregates, year>=2010) %>%
    select(year,
           g_whoregion,
           hivtest_pos,
           hiv_art,
...
if(show_estimates){
  ...
  f2.3.7_txt <- coverage_country[c(1,29),] %>% # tentative China no data
  ...
  # global
  f2.3.7_txt2 <- coverage_global
  f2.3.7_txt3 <- coverage_region %>% 
    arrange(desc(c_art)) %>%
    slice(1,6)
...
f2.3.13_hpmz_txt <- f2.3.13_data %>%
  filter(nrr_hpmz_used==1) %>% nrow()

f2.3.13_txt_hpmz_list <- f2.3.13_data |> 
  filter(nrr_hpmz_used==1) |> 
  arrange(country) |> 
  inner_join(countries, by = "iso3") |>
  select(country_in_text_EN)

f2.3.13_2hrze2hr_txt <- f2.3.13_data %>%
  filter(nrr_2hrze2hr_used==1) %>% nrow()

f2.3.13_txt_2hrze2hr_list <- f2.3.13_data |> 
  filter(nrr_2hrze2hr_used==1) |> 
  arrange(country) |> 
  inner_join(countries, by = "iso3") |>
  select(country_in_text_EN)

palatte_fig2.3.13 = c("dodgerblue3","limegreen","blueviolet","#EFF3FF")

# data check for trend
f2.3.13_data_trend <- filter(notification, year >= 2022) %>%
