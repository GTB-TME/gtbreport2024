#' ---
#' title: Data preparation, Global TB Report 2024, webpages 1.1 and 1.2
...
global23=load(here('inc_mort/estimates2023/global2023.rda'))
globalold=global
...
# Read the file which has the global disaggregations of incidence 
# by age group and sex that also include the uncertainty intervals
load(here::here("disaggregation/output/db_estimates_group.Rdata"))
...
#
# For the 2024 report we show the 2025 milestones, which are:
#
...
# Estimated epidemiological burden of TB in 2023 for 30 high TB burden countries,
...
inc.men.lo=sum(globsplt$inc[globsplt$sex=="M"&globsplt$child==F])/global$inc.num[global$year==yr]*inctot.lo
inc.men.hi=sum(globsplt$inc[globsplt$sex=="M"&globsplt$child==F])/global$inc.num[global$year==yr]*inctot.hi

inc.women=sum(globsplt$inc[globsplt$sex=="F"&globsplt$child==F])/global$inc.num[global$year==yr]*inctot
inc.women.lo=sum(globsplt$inc[globsplt$sex=="F"&globsplt$child==F])/global$inc.num[global$year==yr]*inctot.lo
inc.women.hi=sum(globsplt$inc[globsplt$sex=="F"&globsplt$child==F])/global$inc.num[global$year==yr]*inctot.hi

inc.child=sum(globsplt$inc[globsplt$child==T])/global$inc.num[global$year==yr]*inctot
inc.child.lo=sum(globsplt$inc[globsplt$child==T])/global$inc.num[global$year==yr]*inctot.lo
inc.child.hi=sum(globsplt$inc[globsplt$child==T])/global$inc.num[global$year==yr]*inctot.hi
...
excess=sum(dta$mort.diff.num[dta$year==2020],dta$mort.diff.num[dta$year==2021],dta$mort.diff.num[dta$year==2022],dta$mort.diff.num[dta$year==yr])
excess.lo=sum(dta$mort.diff.lo.num[dta$year==2020],dta$mort.diff.lo.num[dta$year==2021],dta$mort.diff.lo.num[dta$year==2022],dta$mort.diff.lo.num[dta$year==yr])
excess.hi=sum(dta$mort.diff.hi.num[dta$year==2020],dta$mort.diff.hi.num[dta$year==2021],dta$mort.diff.hi.num[dta$year==2022],dta$mort.diff.hi.num[dta$year==yr])
