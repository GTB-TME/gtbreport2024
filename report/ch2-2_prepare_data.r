...
f2.2.1_txt <- filter(f2.2.1_data, entity == "Global" & year >= report_year-6) %>%
...
f2.2.1_region_txt <- filter(f2.2.1_data, year>=report_year-4 & entity %in% c("Region of the Americas", "African Region")) %>%
...
f2.2.3_data_outliers %>% select(income,outlier) %>% arrange(income)%>% ungroup() %>%  toJSON("rows") 

# [{"income":"Low-income","lower":38.4372,"q1":64.8201,"median":70.9892,"q3":83.1626,"upper":97.2668,"color":"#66C2A5"},{"income":"Lower-middle-income","lower":43.6194,"q1":61.8552,"median":75.5734,"q3":85.0298,"upper":94.1131,"color":"#FC8D62"},{"income":"Upper-middle-income","lower":49.0385,"q1":67.6742,"median":78.0552,"q3":85.2173,"upper":95.0068,"color":"#8DA0CB"},{"income":"High-income","lower":59.0226,"q1":78.3224,"median":85.4148,"q3":92.8068,"upper":97.3684,"color":"#E78AC3"}] 
...
f2.2.4_txt_list_hi <- filter(f2.2.4_data, year==report_year-1 & bacconf_pct > 74) %>%
  select(country)
f2.2.4_txt_list_lo <- filter(f2.2.4_data, year==report_year-1 & bacconf_pct < 55) %>%
  select(country) %>%
  arrange(country)
...
f2.2.5_txt <- filter(notification, year  >= report_year - 4) %>%
  select(year,
         g_whoregion,
         c_newinc,
         rdx_data_available,
...
f2.2.8_country_txt <- filter(f2.2.8_data, !is.na(hbc30), wrd_pct>=50)

# previous year's data
f2.2.8_prev_data <- strategy %>%
...
f2.2.8_prev_txt <- f2.2.8_prev_data %>%
  group_by(year) %>%
  summarise(median = median(wrd_pct, na.rm=TRUE)) %>%
  ungroup() %>%
  pivot_wider(names_from = year,
...
# for HBC plotting
f2.2.9_data <- wrd_country %>%
  filter(iso3 %in% hbc30$iso3) %>%
  select(entity = country,
         year,
         pos_wrd_pct = pos_wrd_pct) 

f2.2.9_latest_data <- f2.2.9_data %>%
  filter(year == report_year-1) 
  
max <- max(f2.2.9_latest_data$pos_wrd_pct,na.rm=T)
min <- min(f2.2.9_latest_data$pos_wrd_pct,na.rm=T)

f2.2.9_txt_minmax <- f2.2.9_latest_data %>%
  filter(pos_wrd_pct==max | pos_wrd_pct==min) 

f2.2.9_txt_minmax %>% slice(1) %>% select(entity)
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Data: figure 2.2.10 ----
...
# Calculate regional aggregates
f2.2.10_data <- TBHIV_for_aggregates %>%
  filter(year >= 2010) %>%
  select(g_whoregion,
         year,
...
f2.2.10_data <- rbind(f2.2.10_data, hivstatus_global) %>%
...
f2.2.10_txt <- filter(f2.2.10_data, year>=report_year-3 & entity %in% c("African Region", "European Region", "Global")) %>%
  select(year, entity, hivstatus_pct) %>%
  pivot_wider(names_from = c(entity, year),
              values_from = hivstatus_pct) %>%
  # handle spaces in column names
  rename(AFR_2021 = `African Region_2021`,
...
f2.2.10_txt <- filter(TBHIV_for_aggregates, year == report_year-1) %>%
...
f2.2.11_data <- TBHIV_for_aggregates %>%
  filter(year >= report_year - 2) %>%
  select(iso3,
         country,
         year,
...
# Find the countries with empty data for latest year and see if there are data for the previous year
hivstatus_prev_year_data <- f2.2.11_data %>%
  filter(year == report_year - 1 & is.na(hivstatus_pct)) %>%
  select(iso3) %>%
  inner_join(filter(f2.2.11_data, year == report_year - 2)) %>%
  filter(!is.na(hivstatus_pct))

# Now combine into one dataframe, with previous data used if latest year's data are not available
f2.2.11_data <- f2.2.11_data %>%
  filter(year == report_year - 1) %>%
  anti_join(hivstatus_prev_year_data, by= "iso3") %>%
  rbind(hivstatus_prev_year_data)

# Summary data for text
f2.2.11_txt <- filter(f2.2.11_data, hivstatus_pct >= 90) %>%
  summarise(over_90 = n())

f2.2.11_txt_afr <- filter(f2.2.11_data, hivstatus_pct >= 90) %>%
  filter(g_whoregion == "AFR") %>% nrow()

f2.2.11_txt_lo_list <- filter(f2.2.11_data, hivstatus_pct <= 50) %>% arrange(country)
f2.2.11_txt_lo <- filter(f2.2.11_data, hivstatus_pct <= 50) %>% nrow()
...
dr_surveillance_RUS <- dr_surveillance %>%
  filter(iso3=="RUS")
...
# COmbine regional with global
f2.2.12_data <- rbind(f2.2.12_data, dst_global) %>%
...
# summary data for text
f2.2.12_txt <- filter(f2.2.12_data, year >= report_year-3 & entity %in% c("European Region", "Global")) %>%
...
f2.2.12_txt <- filter(notification, year >= report_year - 2) %>%
  select(year,
         c_newinc,
         conf_rr_nfqr,
         conf_rr_fqr,
...
# Find the countries with empty data for latest year and see if there are data for the previous year
dst_prev_year_data <- f2.2.13_data %>%
  filter(year == report_year - 1 & is.na(dst_pct)) %>%
  select(iso3) %>%
  inner_join(filter(f2.2.13_data, year == report_year - 2), by = "iso3") %>%
...
f2.2.13_txt <- filter(f2.2.13_data, dst_pct >=79.5 & iso3 %in% hbmdr30$iso3) %>%
  arrange(country) %>%
  select(country)

f2.2.13_low_txt <- filter(f2.2.13_data, dst_pct <79.5 & iso3 %in% hbmdr30$iso3) %>%
  arrange(country)
...
# Add global to the regional aggregates
f2.2.14_data <- rbind(f2.2.14_data, fqdst_global) %>%
...
f2.2.14_txt <- filter(notification, year >= (report_year - 3)) %>%
  select(year,
         g_whoregion,
         c_notified,
         c_newinc,
