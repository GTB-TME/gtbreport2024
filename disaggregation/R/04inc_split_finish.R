...
load(here("disaggregation/loutput/incsplits/data/tbimp.Rdata")) # notifications with child splits
load(here("disaggregation/R/02children/data/K.Rdata")) # child estimates
load(here("disaggregation/loutput/incsplits/data/incsplit.Rdata")) # incidnce splits
  ...
  regional <- est[,
    {
      ...
      inc.lo.num = inc.num - 1.96 * ss
      inc.hi.num = inc.num + 1.96 * ss
  ...
  global <- est[,
    {
      inc.num = sum(inc.num)
      ss = sqrt(sum((inc.hi.num - inc.lo.num)^2)) / 3.92
      inc.lo.num = inc.num - ss * 1.96
      inc.hi.num = inc.num + ss * 1.96
      list(inc.num = inc.num, inc.lo.num = inc.lo.num, inc.hi.num = inc.hi.num)
    },
    by = .(year)
  ]
...
if ("inc.num" %ni% names(est)) {
  ...
  est[, c(
    ...
    "inc.lo.num",
    "inc.hi.num"
  ) := .(pop * inc * 1e-5, pop * inc.lo * 1e-5, pop * inc.hi * 1e-5)]
}
...
## ## end temp
db2[, prop := inc / (sum(inc, na.rm = narm) + 1e-15)] # renormalize to whole world
db2a <- db2[, .(prop = sum(prop, na.rm = narm)), by = .(g.whoregion, year, sex, age_group)]
db2a[, pattern := "a"]
db2a[, sum(prop, na.rm = narm)] # check

## other patterns
db2[, kid := ifelse(age_group %in% c("0-4", "5-14"), "0-14", "15plus")] # kid or not
db2b <- db2[, .(prop = sum(prop, na.rm = narm)), by = .(g.whoregion, year, sex, age_group = kid)]
db2[, kid := NULL] # whoregion/sex/<>15
db2b[, pattern := "b"]
db2b[, sum(prop, na.rm = narm)] # check
db2c <- db2b[, .(prop = sum(prop)), by = .(g.whoregion, year, age_group)]
db2c[, pattern := "c"]
db2c[, sex := "a"]
db2c[, sum(prop, na.rm = narm)] # whoregion/<>15
db2d <- db2b[, .(prop = sum(prop)), by = .(g.whoregion, year, sex)]
db2d[, pattern := "d"]
db2d[, age_group := "a"]
db2d[, sum(prop, na.rm = narm)] # whoregion/sex
db2e <- db2d[, .(prop = sum(prop, na.rm = narm)), by = .(g.whoregion, year)]
db2e[, c("age_group", "sex", "pattern") := list("a", "a", "e")]
db2e[, sum(prop, na.rm = narm)] # whoregion
...
dbf[, sum(prop, na.rm = narm), by = pattern] # check


dbf <- merge(dbf, regional[year == estyr, .(
  g.whoregion, inc.num,
  inc.lo.num, inc.hi.num
)],
by = "g.whoregion",
...
dbf[, prop := prop / sum(prop, na.rm = narm),
  by = .(g.whoregion, pattern)
] # normalize by pattern x whoregion


## make the relevant estimates
dbf[, bsd := 0.5 * (inc.hi.num - inc.lo.num) / (1e-15 + inc.num * sqrt(sum(prop^2, na.rm = narm))),
  by = .(pattern, g.whoregion)
]
# disaggregate so as to sum
dbf[, best := inc.num * prop]
dbf[, lo := best * pmax(0, 1 - bsd)]
dbf[, hi := best * (1 + bsd)]
...
#'
## sums by region
dbf[, .SD[, .(test = sum(best, na.rm = narm), aggr = inc.num[1])], by = .(g.whoregion, pattern)]


## uncertainty aggregation:
dbf[, F2 := ((inc.hi.num - inc.lo.num) / inc.num)^2]
(dbf[, .(Fsq = sum(prop^2 * (hi - lo)^2 / best^2, na.rm = narm), F2 = F2[1]),
  by = .(g.whoregion, pattern)
])
...
## global decomposion
dbf[, sum(prop, na.rm = narm), by = .(g.whoregion, pattern)]
dbf[, incR := inc.num[1], by = .(g.whoregion, pattern)] # regional incidence
dbf[, propR := incR / dbf[sex == "a" & age_group == "a", sum(incR, na.rm = narm)]] # propn in region
db3f <- dbf[, .(prop = sum(propR * prop, na.rm = narm)), by = .(year, sex, age_group, pattern)]
db3f <- cbind(db3f, global[year == estyr, .(inc.num, inc.lo.num, inc.hi.num)]) # global data
db3f[, bsd := 0.5 * (inc.hi.num - inc.lo.num) / (1e-15 + inc.num * sqrt(sum(prop^2, na.rm = narm))),
  by = .(pattern)
]
# disaggregate so as to sum
...
db3f[, best := inc.num * prop]
db3f[, lo := best * pmax(0, 1 - bsd)]
db3f[, hi := best * (1 + bsd)]
...
## checks
db3f[, .SD[, .(test = sum(best, na.rm = narm), inc.num[1])], by = .(pattern)] # check
db3f[, F2 := ((inc.hi.num - inc.lo.num) / inc.num)^2]
db3f[, .(Fsq = sum(prop^2 * (hi - lo)^2 / best^2, na.rm = narm), F2 = F2[1]), by = .(pattern)]
...
tmp1 <- db2o[group_name != "global",
  .(suminc = sum(best, na.rm = narm)),
  by = .(age_group, sex)
]
tmp2 <- db2o[group_name == "global",
  .(glob = sum(best, na.rm = narm)),
  by = .(age_group, sex)
]
(tmp <- merge(tmp1, tmp2))
...
fn2 <- here("disaggregation/output")
if(!file.exists(fn2)) dir.create(fn2)
fn2 <- here("disaggregation/output/db_estimates_group.Rdata")
...
db1b <- db1a[, .(prop = sum(prop, na.rm = narm)), by = .(iso3, sex, age_group = kid)]
db1a[, kid := NULL]
db1b[, pattern := "b"]
db1c <- db1b[, .(prop = sum(prop, na.rm = narm)), by = .(iso3, age_group)]
db1c[, sex := "a"]
db1c[, pattern := "c"]
db1d <- db1b[, .(prop = sum(prop, na.rm = narm)), by = .(iso3, sex)]
db1d[, age_group := "a"]
db1d[, pattern := "d"]
db1e <- db1d[, .(prop = sum(prop, na.rm = narm)), by = .(iso3)]
...
dbreg <- dbreg[, .(prop = weighted.mean(prop, inc.num, na.rm = narm)),
  by = .(g.whoregion, sex, age_group, pattern)
] # reg averages
## countries with data
db1f <- merge(db1f, est[year == estyr][, .(
  iso3, year, inc.num,
  inc.lo.num, inc.hi.num
)],
by = "iso3", all.x = TRUE
...
## uncertainty
db1f[, bsd := 0.5 * (inc.hi.num - inc.lo.num) / (1e-15 + inc.num * sqrt(sum(prop^2, na.rm = narm))),
  by = .(iso3, pattern)
]
db1f[, best := inc.num * prop]
db1f[, lo := best * pmax(0, 1 - bsd)]
db1f[, hi := best * (1 + bsd)]
db1f[, c("measure", "unit") := .("inc", "num")]


## checks
db1f[, .(nm = sum(prop, na.rm = narm)), by = .(pattern, iso3)][, summary(nm)]
...
db1f[, .(Fsq = sum(prop^2 * (hi - lo)^2 / (1e-12 + best)^2, na.rm = narm), F2 = F2[1]),
  by = .(iso3, pattern)
]
...
tmpA <- merge(tmpA, est[year == estyr][, .(
  iso3, inc.num,
  inc.lo.num, inc.hi.num
)])
...
fn <- here("disaggregation/output/A_country_incidence_disaggregated_age_sex_num.Rdata")
...
fn <- here("disaggregation/output/A_country_incidence_disaggregated_age_sex_rate.Rdata")
...
dnmz <- c(t(outer(
  c("inc.num.f", "inc.num.m"),
  c("", ".lo", ".hi"), paste0
)))
cnmz <- c(t(outer(
  c("inc.num.15plus", "inc.num.014"),
  c("", ".lo", ".hi"), paste0
)))
bnmz <- c(t(outer(
  c(
    "inc.num.15plus.f", "inc.num.15plus.m",
...
tmpC <- tmpC[, .(
  g.whoregion, best_15plus_a, lo_15plus_a, hi_15plus_a,
...
fn <- here("disaggregation/output/A_regional_incidence_disaggregated_age_sex.Rdata")
...
Tmpc <- Tmpc[, .(
  group_name, best_15plus_a, lo_15plus_a, hi_15plus_a,
  `best_0-14_a`, `lo_0-14_a`, `hi_0-14_a`
)]
names(Tmpc)[2:ncol(Tmpc)] <- cnmz
...
fn <- here("disaggregation/output/global_incidence_disaggregated_age_sex.Rdata")
...
#' # Extra age splits
#'
#' Not looking at additional adolescent age splits, i.e. introducing
#' age groups  15-19, 20-24 from current 15-24
#'
#' First look at which countries have data in these finer age groups
...
look <- c(
  ...
  "newrel.m1519", "newrel.m2024", "newrel.m1524",
  "newrel.f1519", "newrel.f2024", "newrel.f1524"
)


splitd <- tb[year == estyr, ..look][!is.na(newrel.m1519)]

#' inspect:
splitd[, .(newrel.m1519, newrel.m2024, newrel.m1524)]
splitd[, .(newrel.f1519, newrel.f2024, newrel.f1524)]

#' existing NAs are consistent with being coded as zero:
splitd[is.na(newrel.m1519), newrel.m1519 := 0L]
splitd[is.na(newrel.m2024), newrel.m2024 := 0L]
splitd[is.na(newrel.m1524), newrel.m1524 := 0L]
splitd[is.na(newrel.f1519), newrel.f1519 := 0L]
splitd[is.na(newrel.f2024), newrel.f2024 := 0L]
splitd[is.na(newrel.f1524), newrel.f1524 := 0L]

#' test
splitd[, all(newrel.m1519 + newrel.m2024 == newrel.m1524)]
splitd[, all(newrel.f1519 + newrel.f2024 == newrel.f1524)]

#' some issues for SRB only
splitd[(newrel.m1519 + newrel.m2024 != newrel.m1524), .(iso3, newrel.m1519, newrel.m2024, newrel.m1524)]
splitd[(newrel.f1519 + newrel.f2024 != newrel.f1524), .(iso3, newrel.f1519, newrel.f2024, newrel.f1524)]

#' use the sum as the total
splitd[(newrel.m1519 + newrel.m2024 != newrel.m1524), newrel.m1524 := newrel.m1519 + newrel.m2024]
splitd[(newrel.f1519 + newrel.f2024 != newrel.f1524), newrel.f1524 := newrel.f1519 + newrel.f2024]

#' test
splitd[, all(newrel.m1519 + newrel.m2024 == newrel.m1524)] #OK
splitd[, all(newrel.f1519 + newrel.f2024 == newrel.f1524)] #OK


#' what fraction are largeish?
splitd[, summary(newrel.m1524)]
splitd[, mean(newrel.m1524>10)]
splitd[, summary(newrel.f1524)]
splitd[, mean(newrel.f1524 > 10)]

## NOTE save because used in mortality split also
save(splitd, file = here("disaggregation/loutput/splitd.Rdata"))



#' Look at female split
dat <- splitd[, .(
  id = iso3, xi = newrel.f1519, ni = newrel.f1519 + newrel.f2024,
  g.whoregion
)]
...
res.rma <- rma.glmm(
  ...
  xi = xi, ni = ni, data = dat
) # regional

## plot
with(dat, forest(yi,
  ci.lb = ci.lb, ci.ub = ci.ub,
  refline = predict(res, transf = transf.ilogit)$pred, slab = id
))
addpoly(res, row = -1, transf = transf.ilogit)
title("Proportion female 15-19/15-24")
abline(h = 0)

(FS <- predict(res, transf = transf.ilogit, digits = 3))


## by region
FSR <- predict(res.rma, transf = transf.ilogit, digits = 3)
FSR <- data.table(
  mid = FSR$pred, hi = FSR$cr.ub, lo = FSR$cr.lb,
  g.whoregion = dat$g.whoregion
)
(FSR <- FSR[, .(mid = mid[1], hi = hi[1], lo = lo[1]), by = g.whoregion])
fwrite(FSR, file = here("disaggregation/R/02children/data/FSR.csv"))



#' Look at male split
dat <- splitd[, .(
  id = iso3, xi = newrel.m1519, ni = newrel.m1519 + newrel.m2024,
  g.whoregion
)]
...
addpoly(res, row = -1, transf = transf.ilogit)
title("Proportion male 15-19/15-24")
abline(h = 0)

(MS <- predict(res, transf = transf.ilogit, digits = 3))

## by region
MSR <- predict(res.rma, transf = transf.ilogit, digits = 3)
MSR <- data.table(
  mid = MSR$pred, hi = MSR$cr.ub, lo = MSR$cr.lb,
  g.whoregion = dat$g.whoregion
)
(MSR <- MSR[, .(mid = mid[1], hi = hi[1], lo = lo[1]), by = g.whoregion])
fwrite(MSR, file = here("disaggregation/R/02children/data/MSR.csv"))


#' only WPR looks different; what countries int this data are there:
splitd[g.whoregion == "WPR", unique(iso3)]

#' capture split data
splitd[, frac.m := newrel.m1519 / (newrel.m1519 + newrel.m2024)]
splitd[, frac.f := newrel.f1519 / (newrel.f1519 + newrel.f2024)]

regfrax <- merge(
  MSR[, .(g.whoregion, frac.m = mid)],
  FSR[, .(g.whoregion, frac.f = mid)],
  by = "g.whoregion"
)

regfraxl <- melt(regfrax, id = "g.whoregion")
regfraxl <- regfraxl[, .(g.whoregion, sex = gsub("frac\\.", "", variable), regfrac = value)]


#'
#' ## Logic for split
#'
#' Use this logic for split:
#' 1. If 15-19 data available & overall incsplit is based on notifications, use notifications
#' 2. Otherwise, use regional meta-analysis
#'
...
## parent methods
MTHD <- unique(incsplit[,.(iso3,src)])

## load the country estimates
fn2 <- here("disaggregation/output/db_estimates_country.Rdata")
load(file = fn2)
esplit <- db_estimates_country[
  age_group == "15-24" & sex != "a",
  .(iso3, sex, best, lo, hi)
]
esplit <- merge(esplit, unique(tb[, .(iso3, g.whoregion)]), by = "iso3") #add in WHO region
esplit[, length(unique(iso3))]


## reformat for merge
tmp <- melt(splitd[, .(iso3, frac.m, frac.f)], id = "iso3")
tmp <- tmp[, .(iso3, sex = gsub("frac\\.", "", variable), frac = value)]
...
esplit <- merge(
  esplit,
  regfraxl,
  by = c("g.whoregion", "sex"),
  all.x = TRUE
)
esplit <- merge(esplit, MTHD, by = "iso3", all.x = TRUE)

#' apply logic
esplit[src == "Notifications x factor" & is.finite(frac), f := frac] # 1 bullet
esplit[!is.finite(f), f := regfrac] # 2 bullet
esplit[,Fsq:=(hi-lo)^2] # for uncertainty


#' create mid-point estimates & uncertainty work
esplit[, c("best.15_19", "best.20_24") := .(f * best, (1 - f) * best)]
esplit[, c("V.15_19", "V.20_24") := .(f * Fsq, (1 - f) * Fsq)]
esplit[, c("lo.15_19", "hi.15_19", "lo.20_24", "hi.20_24") :=
  .(
    best.15_19 - sqrt(V.15_19) / 2, best.15_19 + sqrt(V.15_19) / 2,
    best.20_24 - sqrt(V.20_24) / 2, best.20_24 + sqrt(V.20_24) / 2
  )]

## shift where necessary
esplit[lo.15_19 < 0, hi.15_19 := hi.15_19 - lo.15_19]
esplit[lo.15_19 < 0, lo.15_19 := 0.0]
esplit[lo.20_24 < 0, hi.20_24 := hi.20_24 - lo.20_24]
esplit[lo.20_24 < 0, lo.20_24 := 0.0]

## test aggregations
esplit[, all(abs(best.15_19 + best.20_24 - best)<1e-4)]
esplit[, all(abs((lo.15_19 - hi.15_19)^2 + (lo.20_24 - hi.20_24)^2 - (lo - hi)^2) < 1e-4)]

#' restructure
esplitr <- esplit[, .(
  iso3, sex,
  best.15_19, best.20_24,
  lo.15_19, lo.20_24,
  hi.15_19, hi.20_24
)]
esplitr <- melt(esplitr, id = c("iso3", "sex"))
esplitr[, c("variable", "acat") := tstrsplit(variable, split = "\\.")]
esplitr[, age_group := gsub("_", "-", acat)]
esplitr <- dcast(esplitr, iso3 + sex + age_group ~ variable, value.var = "value")

#' make both sex aggregation
all <- esplitr[,.(best=sum(best),V=sum((hi-lo)^2)),by=.(iso3,age_group)]
all[, c("lo", "hi") := .(best - sqrt(V) / 2, best + sqrt(V) / 2)]
all[lo < 0, hi := hi - lo]
all[lo < 0, lo := 0.0]
all[,V:=NULL]
all[, sex := "a"]
...
#' add in
esplitr <- rbind(esplitr, all, use.names = TRUE)
esplitr[, c("year", "measure", "unit", "analysed") := .(estyr, "inc", "num", TRUE)]
db_estimates_country_ado <- db_estimates_country[age_group != "15-24"]
db_estimates_country_ado <- rbind(db_estimates_country_ado, esplitr, use.names = TRUE)


#' regional & global
esplitr <- merge(esplitr,
                 unique(tb[, .(iso3, g_whoregion = g.whoregion)]), by = "iso3") # add in WHO region

## global
glob <- esplitr[, .(best = sum(best), V = sum((hi - lo)^2)), by = .(sex, age_group)]
glob[, c("lo", "hi") := .(best - sqrt(V) / 2, best + sqrt(V) / 2)]
glob[lo < 0, hi := best - lo]
glob[lo < 0, lo := 0]
glob[, g_whoregion := "global"]
glob[, c("group_type", "group_name") := .("global", "global")]

## reg
reg <- esplitr[, .(best = sum(best), V = sum((hi - lo)^2)), by = .(sex, age_group, g_whoregion)]
reg[, c("lo", "hi") := .(best - sqrt(V) / 2, best + sqrt(V) / 2)]
reg[lo < 0, hi := best - lo]
reg[lo < 0, lo := 0]
reg[, c("group_type", "group_name") := .("g_whoregion", g_whoregion)]

xtra <- rbind(reg,glob)
xtra <- xtra[, .(group_type, group_name,
  year = estyr,
  measure = "inc", unit = "num",
  age_group, sex,
  best, lo, hi
)]

db_estimates_group_ado <- db_estimates_group[age_group != "15-24"]
db_estimates_group_ado <- rbind(db_estimates_group_ado, xtra, use.names = TRUE)


#' save out:

attr(db_estimates_country_ado, "timestamp") <- Sys.Date() # set date
fn2 <- here("disaggregation/output/db_estimates_country_ado.Rdata")
save(db_estimates_country_ado, file = fn2)

attr(db_estimates_group_ado, "timestamp") <- Sys.Date() # set date
fn2 <- here("disaggregation/output/db_estimates_group_ado.Rdata")
save(db_estimates_group_ado, file = fn2)
