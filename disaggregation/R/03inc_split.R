...
load(here("disaggregation/loutput/prevsplits/prevsplits.Rdata")) # from prevalence split work
...
if (noests.yet) {
  load(here("disaggregation/tmpdata/est.rda"))
  xtra <- est[year == estyr - 1]
  xtra[, year := estyr]
  est <- rbind(est, xtra)
} else {
...
drnm <- here("disaggregation/loutput/incsplits/")
...
glop <- prevsplits[, .(
  propm = median(prop, na.rm = TRUE), # global median
  propm.se = median(prop.se, na.rm = TRUE)
),
by = .(sex, age_group)
]
tot <- glop[, sum(propm)]
glop[, propm := propm / tot] # renormalize
prevsplits <- merge(prevsplits, glop,
  by = c("sex", "age_group"),
  all.x = TRUE, all.y = TRUE, allow.cartesian = TRUE
)
prevsplits[
  iso3 %in% missed,
  c("prop", "prop.se") := list(propm, propm.se)
] # fill out with global version
...
YSR <- data.table(
  mid = YSR$pred, hi = YSR$cr.ub, lo = YSR$cr.lb,
...
## ======== 5-14 ===== proportion M
dat <- tmpw[, .(
  id = iso3, xi = newrel.m514, ni = newrel.m514 + newrel.f514,
  g.whoregion
)]
dat <- dat[dat$ni > 0, ]
dat <- dat[dat$ni > dat$xi, ]
...
OSR <- data.table(
  mid = OSR$pred, hi = OSR$cr.ub, lo = OSR$cr.lb,
  g.whoregion = dat$g.whoregion
)
...
drnm <- here("disaggregation/loutput/incsplits/data/")
if(!file.exists(drnm)) dir.create(drnm)
cat(knotem.now, file = here("disaggregation/loutput/incsplits/data/knotem.now.txt"))
cat(knotem.then, file = here("disaggregation/loutput/incsplits/data/knotem.then.txt"))
...
tmp <- tb[
  year == estyr & !is.na(newrel.m04) & !is.na(newrel.m514),
...
fwrite(tmp, file = here("disaggregation/loutput/incsplits/data/knotem.csv"))
...
tb[
  year == estyr & !is.na(newrel.m014) & is.na(newrel.m04),
...
cat(knotef.now, file = here("disaggregation/loutput/incsplits/data/knotef.now.txt"))
cat(knotef.then, file = here("disaggregation/loutput/incsplits/data/knotef.then.txt"))
...
tb[year == estyr & !is.na(newrel.f014) & is.na(newrel.f04) &
  newrel.f014 == 0, ..see]
tb[
  year == estyr & !is.na(newrel.f014) & is.na(newrel.f04) &
    newrel.f014 == 0,
...
#' For countries where there is a total count, assign to categories by regional weight
tmp <- tb[
  year == estyr & !is.na(newrel.f04) & !is.na(newrel.f514),
  .(g.whoregion, newrel.f04, newrel.f514)
]


ggplot(
  data = tmp[newrel.f04 > 0, .(frac = newrel.f04 /
...
fwrite(tmp, file = here("disaggregation/loutput/incsplits/data/knotef.csv"))
...
#'
#' Inspect:
am <- paste0("newrel.m", agz)
af <- paste0("newrel.f", agz)
tbnow <- tb[year == estyr]
...
tbnow[, tots := tots]
toshowm <- c("iso3", "tots", am)
toshowf <- c("iso3", "tots", af)
...
cat(naz, file = here("disaggregation/loutput/incsplits/data/naz.txt"))
...
for (j in ind) {
  set(tb,
    i = which(tb$year == estyr & tb$iso3 %in% naz & is.na(tb[[j]])),
    j = j, value = 0
  )
}
...
tb[iso3 == "UGA" & year == 2022, c("newrel.f5564", "newrel.f65")
:= .(newrel.f5564 * resf, newrel.f5564 * (1 - resf))]
tb[iso3 == "UGA" & year == 2022, c("newrel.m5564", "newrel.m65")
:= .(newrel.m5564 * resm, newrel.m5564 * (1 - resm))]
tb[iso3 == "UGA" & year == 2022, .(newrel.m5564, newrel.m65, newrel.f5564, newrel.f65)]



#' Keep this version of `tb.Rdata` in case needed for CFR mortality
save(tb, file = here("disaggregation/loutput/incsplits/data/tbimp.Rdata"))
...
#'
drnm <- here("disaggregation/loutput/incsplits/plots")
if(!file.exists(drnm)) dir.create(drnm)
fz <- list.files(
  path = drnm,
  pattern = "*.pdf",
...
#'
isopass1 <- c(
  cdrtab[isosf][cdr > 0.85, iso3], # SF + CDR>0.85
...
cat(isopass1, file = here("disaggregation/loutput/incsplits/data/pass1.txt"))


#' Create directory(s) if missing
drnm <- here("disaggregation/loutput/incsplits/plots/pass1")
if(!file.exists(drnm)) dir.create(drnm)
...
cat(noteissue, file = here("disaggregation/loutput/incsplits/data/noteissue.txt"))
...
priorsample <- function(cn,
  ...
  if(verbose) silent <- FALSE                       #for consistency
  ...
  if(tmp$year[1] < max(tb$year)){# if not most recent year then rescale
    if(verbose) cat('...rescaling as notes year < estyr!\n')
    EL <- est[iso3==cn][,.(iso3,year,inc,pop)]
  ...
  INC <- est[iso3==cn][year==estyr,inc*pop/1e5] #total incidence
  atmp <- tmp[age_group %ni% kds,]    #adult notifications
  tmp2 <- K[cn]
  if (!is.finite(INC)) stop("Country total incidence estimate not finite!!")
  if (nrow(atmp)==0) stop("Country lacks adult notification rows!")
  if(nrow(tmp2)==0) stop('Country not found in K!')
...
#' test!
tmp <- getNotes("NGA", silent = FALSE)
niplot(tmp)
tmp <- priorsample("NGA", verbose = TRUE)
niplot(tmp)
...
drnm <- here("disaggregation/loutput/incsplits/plots/pass2")
if(!file.exists(drnm)) dir.create(drnm)
...
moz <- niplot(tmp3)
ggsave(moz,
  device = cairo_pdf, w = W, h = H,
  filename = here("disaggregation/loutput/incsplits/plots/pass2/MOZ2b.pdf")
)
...
## 2024
## NOTE SYR now in pass 1

isopass3 <- c(
  "BFA", # looks unlucky on prior but last yr similar to p3 outcome
  "BGD", #/
  "CHN", #/
  "IDN", #/
  "IND", #/
  "LAO", #/china like pyramid but CDR improved
  "MMR", # ? pass but odd sex split in adolescents
  "PNG", #/
  "PRK", #/
  "RWA", # looks like CDR>1 better via pass 3
  "SEN", # looks unlucky but higher CDR
  "SLE", # pass but unusual tilt MF, higher CDR
  "SSD", # higher CDR causing collision
  "TGO", # CDR>1 better via pass 3
  "TLS", # odd sex balance: better pass 3 given no other evidence
  ## "TZA", # very slightly odd, but not a fail
  ## "UGA", # odd due to odd age groups but pass 3 can't work with these
  "ZMB" #high CDR
)

## NOTE more pass 2 fails in AFR region with higher CDR. Probably reasonable as these are moving towards
## pass 1. Alternatively could work harder on pass 2 sampling framework as some would have passes with
## more efficient sampling.

#' Create directory if missing
drnm <- here("disaggregation/loutput/incsplits/plots/pass3")
if(!file.exists(drnm)) dir.create(drnm)
...
#' Create directory if missing
drnm <- here("disaggregation/loutput/incsplits/plots/pass2/0pass2fails")
if(!file.exists(drnm)) dir.create(drnm)


## move them into the failed directory
for (cn in isopass3) {
  ofn <- here("disaggregation/loutput/incsplits/plots/pass2") + glue("/{cn}2.pdf")
  nfn <- here("disaggregation/loutput/incsplits/plots/pass2/0pass2fails") + glue("/{cn}2.pdf")
...
#' Create directory if missing
drnm <- here("disaggregation/loutput/incsplits/plots/data")
if(!file.exists(drnm)) dir.create(drnm)
...
length(ISL)
length(pltlist)
save(file = here("disaggregation/loutput/incsplits/plots/data/pltlist.Rdata"), pltlist)
save(file = here("disaggregation/loutput/incsplits/plots/data/hi30.Rdata"), hi30)


## aggregations
incsplit <- rbindlist(ISL, fill = TRUE)
incsplit[!is.finite(inc)] # OK -- none
incsplit[!is.finite(inc), c("inc", "prop.se") := 0.0] # remove
...
#' Save out data:
save(
  file = here("disaggregation/loutput/incsplits/data/incsplit.Rdata"),
  incsplit
)
...
#'
#' Load
load(file = here("disaggregation/loutput/incsplits/data/incsplit.Rdata"))
load(file = here("disaggregation/loutput/incsplits/plots/data/hi30.Rdata"))
...
mryr[year != estyr]
fwrite(mryr, file = here("disaggregation/loutput/incsplits/data/mryr.csv"))
...
if (length(hbcna) > 0) {
  cat(hbcna, file = here("disaggregation/loutput/incsplits/data/hbcna.txt"))
  for (cn in hbcna) {
    tmp <- incsplit[cn]
...
#' Save out plot
#'
fn <- here("disaggregation/loutput/incsplits/aplots")
if(!file.exists(fn)) dir.create(fn)
fn <- here("disaggregation/loutput/incsplits/aplots/HI30.pdf")
...
wz[1] <- 1.15
wz <- wz / 1.15
fn <- here("disaggregation/loutput/incsplits/aplots/HI30b.pdf")
...
fn <- here("disaggregation/loutput/incsplits/data/regsplt.Rdata")
...
for (i in seq_along(whoz)) {
  ...
  nplst[[i]] <-
    ...
    geom_bar(
      data = reg[sex == "M"], stat = "identity",
      aes(x = age, y = inc), fill = "transparent", col = 1
    ) +
    geom_bar(
      data = reg[sex == "F"], stat = "identity",
      aes(x = age, y = -inc), fill = "transparent", col = 1
    ) +
    ggtitle(whozt[i]) +
    theme_classic() +
...
fn <- here("disaggregation/loutput/incsplits/aplots/regional.pdf")
...
fn <- here("disaggregation/loutput/incsplits/data/glob.Rdata")
...
plt <- ggplot(data = glob, aes(x = age, y = newrel, fill = sex)) +
  ...
  ylab("New & relapse cases") +
  xlab("Age group") +
  scale_y_continuous(labels = absspace) +
  geom_bar(
    data = glob[sex == "M"], stat = "identity",
    aes(x = age, y = inc), fill = "transparent", col = 1
  ) +
  geom_bar(
    data = glob[sex == "F"], stat = "identity",
    aes(x = age, y = -inc), fill = "transparent", col = 1
  ) +
  theme_classic() +
  theme(legend.position = "none") +
...
fn <- here("disaggregation/loutput/incsplits/aplots/global.pdf")
if (gypt) {
...
names(denom)[2] <- "pop"
summary(denom)
fn <- here("disaggregation/loutput/denom.Rdata")
save(file = fn, denom)
...
h30splt[
  iso3 == "MOZ" & !age %in% c("0-4", "5-14"),
  newrel := NA
] # NOTE this year - as has been split evenly in dx plt

h30splt <- merge(h30splt, denom,
  by = c("iso3", "sex", "age"),
  all.x = TRUE, all.y = FALSE
)

fn <- here("disaggregation/output")
if (!file.exists(fn)) dir.create(fn)
fn <- here("disaggregation/output/h30splt.rda")
...
fn <- here("disaggregation/output/regsplt.rda")
...
fn <- here("disaggregation/output/globsplt.rda")
