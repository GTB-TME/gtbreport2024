...
## qbeta warnings...
system.time({ # around 100 sec for 1e4: less now ~62s in 24
...
BB <- merge(BB, unique(est[, .(iso3, g.whoregion)]),
  by = "iso3",
...
reporting <- function(testdf) { # takes data.table
  ## least disaggregated
  report4 <- testdf[, list(
    median = round(quantile(value, 0.5)),
...
ruq <- function(x, digits = 0) {
  round(quantile(x, probs = 0.75),
    digits = digits
  )
}
rlq <- function(x, digits = 0) {
  round(quantile(x, probs = 0.25),
    digits = digits
  )
}
...
## NOTE file found in 02children/plots/paedout.csv for last year
## check against last year
DO <- fread(here("disaggregation/R/02children/data/paedout_2023.csv"))
...
p1 <- ggplot(data = DO[odd, ], aes(x = country, y = median / M14)) +
  ...
  ylab("Ratio 2023/2022") + # as years
  ggtitle("Countries with > 50% change 2022->2023")
p1
ggsave(p1,
  file = here("disaggregation/R/02children/plots/Ratio.pdf"),
  width = 10
)
...
p <- ggplot(data = DO, aes(x = M14, y = median)) +
  ...
  geom_point() +
  xlab("prior 2022 incidence") +
  ylab("prior 2023 incidence")
