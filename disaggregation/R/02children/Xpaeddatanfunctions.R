...
noests.yet <- FALSE
if (noests.yet) {
  load(here("disaggregation/tmpdata/unaids.rda"))
...
if (!file.exists(here("disaggregation/R/02children/data/unaids.Rdata"))) {
  fn <- here("data/unaids/HIV2024Estimates_ForTBmodel_AllCountries_Embargoed22uly_3July2024.xlsx")
  ## reformat:
  ...
  Y <- Y[country != "Global"]
  Y <- Y[, .(year, iso3,
    pop014 = as.numeric(`O- Population aged 0-14 Male+Female`),
    hiv014 = as.numeric(`O- HIV population (0-14) Male+Female`),
    hiv014.lo = as.numeric(`O- HIV population (0-14) Male+Female; Lower bound`),
    hiv014.hi = as.numeric(`O- HIV population (0-14) Male+Female; Upper bound`)
  )]
...
eprev[, c("prev", "prev.sd", "prev.lo", "prev.hi") :=
  .(prev * rat, prev.sd * rat, prev.lo * rat, prev.hi * rat)] # scale
eprev[, rat := NULL]
...
if (nrow(tmp) > 0) { # probably no big contributors; use mean
  print(tmp)
  mbcg <- mean(Dr$coverage, na.rm = TRUE)
  Dr[is.na(coverage), coverage := mbcg]
}
## HIV
...
getIncidence <- function(Dr, parms, ARI, HIV, method = "lat") {
  ...
  if (method == "lat") {
    L <- L - parms$latprop * (1 - LAT / 90)
  } # proportion of protection left @ lat
...
parm <- list(
  ...
  prDTB = c(
    ...
    0.005,
    0.005
  ), # risk of progression TBM/miliary
...
makeParameters2 <- function(LH, mp) {
  ...
  tb[tb > 1e5] <- 1e5
  return(list(
    p1 = p1z, p2 = p2z, vd = vdz, vp = vpz, irr = irrz, b = bz,
    tb = tb, h = hiv
  ))
}
...
loopAns2 <- function(LP, mp, method = c("comm", "lat")) {
  ...
  ans2 <- copy(ans)
  ans4 <- data.table(
    country = rep(Dr$country, n), ariz = 0.0,
