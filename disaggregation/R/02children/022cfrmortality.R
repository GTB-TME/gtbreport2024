...
## ## TODO BUG with UGA currently
## incsplit[iso3 == "UGA"]
## regav <- incsplit[, .(tprop = mean(prop, na.rm = TRUE)), by = .(age_group, sex)]
## regav[, tprop := tprop / sum(tprop)]
## regav[, sum(tprop)]
## tmpi <- est[iso3 == "UGA" & year == estyr, inc.num]
## regav[, ninc := tmpi * tprop]
## incsplit <- merge(incsplit,regav,by=c('age_group','sex'))
## incsplit[iso3 == "UGA", c("prop", "inc") := .(tprop, ninc)]
## incsplit[, c("tprop", "ninc") := NULL]
## setkey(incsplit, iso3) #NOTE needed
## incsplit[iso3 == "UGA"]
...
senspriv <- c(
  ...
  0,
  9
) * 1e-2 # proportions in private sector
...
for (i in 1:nrep) {
  ...
  if (any(is.na(DeathsOffTxY))) stop(paste0("DeathsOffTxY: i=", i, "\n"))
  ...
  if (any(is.na(DeathsOffTxO))) stop(paste0("DeathsOffTxO: i=", i, "\n"))
  ## record results
...
RESATOTS <- RESATOT[, list(
  mid = median(total),
  lo = quantile(total, probs = .025),
  hi = quantile(total, probs = .975)
)] # summarize totals
...
if (!sensNote & !useHIVneg) {
  ...
  abline(v = RESATOTS$mid, col = 2, lwd = 3)
  abline(v = RESATOTS$lo, lty = 2)
  abline(v = RESATOTS$hi, lty = 2)
  dev.off()
}
...
## regional totals
RESAG <- RESA[, list(
  OnTx = sum(DeathsOnTx), OffTx = sum(DeathsOffTx),
...
gwho <- sort(gwho)
txstat <- c(
  "On tx (HIV-)", "Off tx (HIV-)", "On tx (HIV+)",
  "Off tx (HIV+)"
)
...
## youngs ----------
RESATy <- RESA[, list(
  OnTx = sum(DeathsOnTxY), OffTx = sum(DeathsOffTxY),
...
RESATsy <- RESATy[, list(
  OnTx = quantile(OnTx, .5), OffTx = quantile(OffTx, .5),
...
RESAc <- RESA[, list(
  age = mid(DeathsOnTx + DeathsOffTx + DeathsOnTxH +
    DeathsOffTxH),
...
RESAC <- RESA[, .(
  deaths = midr(DeathsOnTx + DeathsOffTx +
    DeathsOnTxH + DeathsOffTxH),
