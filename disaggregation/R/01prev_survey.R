...
prev[age_group %ni% xclude,
  {
    print(paste0(iso3, "=", length(unique(age_group))))
  },
  by = iso3
]
prev[age_group %ni% xclude,
...
ggsave(GP,
  file = here("disaggregation/loutput/prevsplits/PSinput.pdf"),
  w = 26, h = 15
)
...
getGibbs <- function(Z, Z.se, nchain = 1e2,
  ...
  for (i in 1:nchain) {
    ...
    mustore[[i]] <- mu
    ystore[[i]] <- Y
    pstore[[i]] <- mvrnorm(1, mu, Sig)
    ## Sigstore[[i]] <- Sig
  }
  list(muz = mustore, Yz = ystore, ps = pstore)
}

save(PR, file = here("disaggregation/loutput/prevsplits/PR.Rdata")) # save out data to be used
load(file = here("disaggregation/loutput/prevsplits/PR.Rdata"))
...
#' store for later:
agedisag[["AFR"]] <- list(isoz = unlist(lapply(
  strsplit(unlist(Z.df[, 1]), split = " "),
  function(x) x[1]
)), Ylist = out$Yz, Pmat = PZ)
...
makeregionplot <- function(xtradata, reg = "AFR", alph = .1) {
  PP <- ggplot(
    data = PR[g_whoregion == reg],
    aes(
      x = age_group, y = logRR, col = iso3.year, fill = iso3.year,
      group = g_whoregion
    )
  ) +
    facet_wrap(~g_whoregion) +
    ...
    stat_summary(
      data = xtradata, geom = "ribbon", alpha = .2, col = NA,
...
save(PZdfm, file = here("disaggregation/loutput/prevsplits/PZdfm.Rdata"))
...
#' store for later:
agedisag[["SEA"]] <- list(isoz = unlist(lapply(
  strsplit(unlist(Z.df[, 1]), split = " "),
  function(x) x[1]
)), Ylist = out$Yz, Pmat = PZ)
...
#' store for later:
agedisag[["WPR"]] <- list(isoz = unlist(lapply(
  strsplit(unlist(Z.df[, 1]), split = " "),
  function(x) x[1]
)), Ylist = out$Yz, Pmat = PZ)
...
#' store for later:
agedisag[["EMR"]] <- list(isoz = unlist(lapply(
  strsplit(unlist(Z.df[, 1]), split = " "),
  function(x) x[1]
)), Ylist = out$Yz, Pmat = PZ)
...
MFD <- MFD[iso3.year %ni% c(
  "CHN 1990", "CHN 2000", "BGD 2008",
  "IDN 2004", "KOR 1990", "MMR 1994"
), ] # keep most recent
...
KH <- data.table(
  g_whoregion = c("SEA", "WPR", "AFR", "EMR", "AMR", "GLO"),
...
correctRRs <- function(cn, MFs, RRs) {
  if (length(MFs) == 1) {
    return(list(correctRR(cn, MFs, RRs)))
  }
...
for (cn in unique(est$iso3)) {
  if (nrow(est[iso3 == cn & year == estyr, ]) > 0) { # skip things without estimates
    txt <- capture.output(Pdt <- getProps(
      n = 1e2,
      cn = cn,
      verbose = TRUE,
      shape = TRUE
...
cat(prevsplits[is.na(prop), unique(iso3)],
  file = here("disaggregation/loutput/prevsplits/zproblems.txt")
)


#' save out results:
save(
  file = here("disaggregation/loutput/prevsplits/prevsplits.Rdata"),
  prevsplits
)
save(file = here("disaggregation/loutput/prevsplits/prevsrc.Rdata"), prevsrc)
fn <- here("disaggregation/loutput/prevsplits/datal")
...
countryplot <- function(cn) {
  ...
  ans <- ggplot(data = tmp, aes(x = age_group, y = prop, fill = sex)) +
    ...
    geom_errorbar(
      data = tmp[sex == "M", ],
      aes(ymin = prop - 1.96 * prop.se, ymax = prop + 1.96 * prop.se), width = 0
    ) +
    geom_errorbar(
      data = tmp[sex == "F", ],
      aes(ymin = -prop - 1.96 * prop.se, ymax = -prop + 1.96 * prop.se), width = 0
    ) +
    ggtitle(paste0("Adult TB prevalence split: ", cn))
  ans
}
...
#' loop & save out (SLOW)
fn <- here("disaggregation/loutput/prevsplits/plots")
if(!file.exists(fn)) dir.create(fn)
for (cn in unique(prevsplits$iso3)) {
  if (!is.na(prevsplits[iso3 == cn, sum(prop)])) {
    plt <- countryplot(cn)
    fn <- glue(here("disaggregation/loutput/prevsplits/plots")) +
      "/" + cn + ".pdf"
