...
load(here("disaggregation/loutput/incsplits/data/incsplit.Rdata")) # from prevalence split work
  ...
  regional <- est[,
    {
      ...
      list(
        inc.num = inc.num, inc.lo.num = inc.lo.num, inc.hi.num = inc.hi.num,
...
fn <- here("disaggregation/loutput/mortsplits")
if(!file.exists(fn)) dir.create(fn)
fn <- here("disaggregation/loutput/mortsplits/plots")
if(!file.exists(fn)) dir.create(fn)

#' Check which countries to get IHME data for:
fn <- here("disaggregation/loutput/mortsplits/vr_ihme_iso3.txt")
if (!file.exists(fn)) {
...
csvfn <- here("data/mortality/dths_total.csv")


#' big loop to generate cleaned/reshaped file if not there:
mfn <- here("disaggregation/loutput/mortsplits/VR.Rdata")
if (!file.exists(mfn)) {
  ## reading in CSV data
  ## -------- ICD 10
...
getVRMortnh <- function(cn, KMoverride = TRUE, allplt = FALSE) {
  ...
  tmp[
    age_group == "0_4" & sex == "M",
    mort := mfy * km[, deathsY * (1 - h)]
  ] # young,m
  tmp[
    age_group == "0_4" & sex == "F",
    mort := (1 - mfy) * km[, deathsY * (1 - h)]
  ] # young,f
  tmp[
    age_group == "5_14" & sex == "M",
    mort := mfo * km[, (deaths - deathsY) * (1 - h)]
  ] # old,m
  tmp[
    age_group == "5_14" & sex == "F",
    mort := (1 - mfo) * km[, (deaths - deathsY) * (1 - h)]
  ] # old,f
  ## adults
...
missco <- est[year == estyr][, .(
  iso3, inc.num, inc.h.num, inc.nh.num,
...
## save(allcns,file=here('disaggregationloutput/mortsplits/allcns.Rdata'))
save(allcnsH,
  file = here("disaggregation/loutput/mortsplits/plots/allcnsH.Rdata")
)
save(allcnsNH,
  file = here("disaggregation/loutput/mortsplits/plots/allcnsNH.Rdata")
)
save(HNsplit,
  file = here("disaggregation/loutput/mortsplits/plots/HNsplitL.Rdata")
)
save(HPsplit,
  file = here("disaggregation/loutput/mortsplits/plots/HPsplitL.Rdata")
)
...
## HIV-ve
fn <- glue(here("disaggregation/loutput/mortsplits/plots/nhMort_"))
j <- k <- 0
...
## HIV+ve
fn <- glue(here("disaggregation/loutput/mortsplits/plots/hMort_"))
j <- k <- 0
...
save(HNsplit, file = here("disaggregation/loutput/mortsplits/HNsplit.Rdata"))


load(file = here("disaggregation/loutput/mortsplits/HNsplit.Rdata"))
...
save(HPsplit, file = here("disaggregation/loutput/mortsplits/HPsplit.Rdata"))
...
fn <- here("disaggregation/loutput/mortsplits/plots/mu30_NH.pdf")
...
wz <- rep(1, 5)
wz[1] <- 1.05
fn <- here("disaggregation/loutput/mortsplits/plots/mu30_NHb.pdf")
...
fn <- here("disaggregation/loutput/mortsplits/plots/mu30_NHc.pdf")
...
hh <- f * 7.5 / 2
ww <- f * 10 / 3
fn <- here("disaggregation/loutput/mortsplits/plots/muGlobal_NH.pdf")
GP <- muplot(mortgloHN, "Global")
if (gypt) ggsave(filename = fn, GP, height = hh, width = ww, device = cairo_pdf)


## rotated
fn <- here("disaggregation/loutput/mortsplits/plots/muGlobal_NHc.pdf")
GP <- muplot(mortgloHN, "Global") +
  coord_flip() +
  theme(
    axis.title.x = element_blank(),
...
Mglobsplt <- mortgloHN
fn <- here("disaggregation/output/Mglobsplt.rda")
...
## save out data for report plots
Mregsplt <- mortregHN
fn <- here("disaggregation/output/Mregsplt.rda")
...
## save out
fn <- here("disaggregation/loutput/mortsplits/plots/muRegional_HN.pdf")
...
## save out
fn <- here("disaggregation/loutput/mortsplits/plots/muRegional_HNc.pdf")
GP <- ggarrange(plotlist = Pltlst, ncol = 3, nrow = 2)
ggsave(GP, filename = fn, h = 7.5, w = 10, device = cairo_pdf)
...
## write out
fn <- here("disaggregation/loutput/mortsplits/HNregional.csv")
...
## write out
fn <- here("disaggregation/loutput/mortsplits/HPregional.csv")
...
## test sums
HPUR[, .(
  mid = sum(mid), M014.mid = sum(M014.mid), F014.mid = sum(F014.mid),
...
## write out
fwrite(HNglout,
  file = here("disaggregation/loutput/mortsplits/HNglobal.csv")
)
fwrite(HPglout,
  file = here("disaggregation/loutput/mortsplits/HPglobal.csv")
)
...
load(here("disaggregation/loutput/mortsplits/HNsplit.Rdata")) # to be overwritten
...
extras <- est[year == estyr][
  iso3 %ni% HNsplit[, iso3],
...
HNsplit <- rbind(HNsplit[, .(
  iso3, sex,
  age_group, mort,
  src.mort, g.whoregion
)], extras)
HNsplit <- HNsplit[order(as.character(iso3)), ]
save(HNsplit,
  file = here("disaggregation/loutput/mortsplits/HNsplit.Rdata")
)
...
HNsplit[, c(
  "F", "mort.nh.hi.num", "mort.nh.lo.num", "mort.nh.num",
  "mort", "mid", "src.mort"
) := NULL]
...
fn <- here("disaggregation/loutput/mortsplits/db_hn_mortality_disaggregated.Rdata")
save(db_hn_mortality_disaggregated, file = fn)
fn <- here("disaggregation/output/db_hn_mortality_disaggregated.Rdata")
save(db_hn_mortality_disaggregated,file=fn)
...
fn <- here("disaggregation/output/MPglobsplit.rda")
save(MPglobsplit,file=fn)


#' CFR checks:
load(here("disaggregation/loutput/mortsplits/HNsplit.Rdata"))
load(here("disaggregation/loutput/incsplits/data/incsplit.Rdata"))


HNsplit <- merge(HNsplit, est[year == estyr][, .(
  iso3,
  mort.nh.num,
  mort.nh.lo.num,
  mort.nh.hi.num
...
#'
#' # Extra adolescent mortality splits
#'
#'
#' Load parent data
#'
...
## CFRs HIV-negative (from fun.R/inc2mort)
CFRu <- c(0.43, (0.53 - 0.28) / 3.92) # undetected
CFRd <- c(0.03, 0.07 / 3.92) # detected

## check: what fraction of deaths are on tx vs not? (approximately)
tmp <- est[year == estyr, .(iso3,
  undetected = ((inc.num - c.newinc) * CFRu[1]),
  detected = (c.newinc * CFRd[1])
)]
tmp[is.na(detected),iso3]
tmp[, .(
  undetected = sum(undetected, na.rm = TRUE), detected = sum(detected, na.rm = TRUE),
  frac.undetected = sum(undetected, na.rm = TRUE) /
    (sum(undetected, na.rm = TRUE) + sum(detected, na.rm = TRUE))
)] #~80%


#' load existing splits
## disaggregated incidence
fn2 <- here("disaggregation/output/db_estimates_country_ado.Rdata")
load(file = fn2)
## undisaggregated mortality
fn <- here("disaggregation/output/db_hn_mortality_disaggregated.Rdata")
load(file = fn)

## #' existing I & M
## IM <- merge(db_estimates_country[, .(iso3, age_group, sex, inc = best, inc.lo = lo, inc.hi = hi)],
##   db_hn_mortality_disaggregated[, .(iso3, age_group, sex, g.whoregion,
##     mort.nh = best, mort.nh.lo = lo, mort.nh.hi = hi
##   )],
##   by = c("iso3", "age_group", "sex")
## )

#' NOTE because the incidence split is based on notifications, and there is no differential CFR on treatment by age, mortality should be split by the same fraction as incidence

tmp <- db_estimates_country_ado[
  sex != "a" & age_group %in% c("15-19", "20-24"),
  .(iso3, age_group, sex, best)
]
tmp <- dcast(tmp, iso3 + sex ~ age_group, value.var = "best")
tmp[, frac := `15-19` / (`15-19` + `20-24`)]
tmp <- merge(tmp, unique(tb[, .(iso3, g.whoregion)]), by = "iso3")
tmp[, avfrac := mean(frac, na.rm = TRUE), by = .(sex, g.whoregion)]
tmp[!is.finite(frac), frac := avfrac]

## data to be split
tosplit <- db_hn_mortality_disaggregated[age_group == "15-24"]
tosplit <- tosplit[, .(iso3, sex, best, F = (hi - lo)^2)]
tosplit <- merge(tosplit, tmp[, .(iso3, sex, frac)], by = c("iso3", "sex"))
tosplit[, c("best.15_19", "best.20_24") := .(best * frac, best * (1 - frac))]
tosplit[, c("F.15_19", "F.20_24") := .(F * frac, F * (1 - frac))]
tosplit <- melt(tosplit[, .(iso3, sex, best.15_19, best.20_24, F.15_19, F.20_24)],
                id = c("iso3", "sex"))

tosplit[, c("qty", "acat") := tstrsplit(variable, split = "\\.")]
tosplit[, age_group := gsub("_", "-", acat)]
tosplit <- dcast(tosplit[, .(iso3, sex, qty, age_group, value)],
                 iso3 + sex + age_group ~ qty, value.var = "value")

tosplit[, c("lo", "hi") := .(best - sqrt(F) / 2, best + sqrt(F) / 2)]
tosplit[lo < 0, hi := hi - lo]
tosplit[lo < 0, lo := 0]

## extra cols
tosplit <- merge(tosplit, unique(tb[, .(iso3, g.whoregion)]), by = "iso3")
tosplit[, c("year", "measure", "unit") := .(estyr, "mortality", "number")]
tosplit[,F:=NULL]

#' add in
db_hn_mortality_disaggregated_ado <- db_hn_mortality_disaggregated[age_group != "15-24"]
db_hn_mortality_disaggregated_ado <- rbind(db_hn_mortality_disaggregated_ado,
                                           tosplit,
                                           use.names=TRUE)
...
#' tests
#'
test1 <- db_hn_mortality_disaggregated_ado[age_group %in% c("15-19", "20-24"),
  .(
    best1 = sum(best),
    V1 = sum((hi - lo)^2)
  ),
...
test2 <- db_hn_mortality_disaggregated[age_group %in% c("15-24"),
  .(
    best2 = sum(best),
    V2 = sum((hi - lo)^2)
  ),
...
test <- merge(test1, test2, by = "iso3")
test[,all(abs(best1-best2)< 1e-2)] #OK
test[, all(abs(V1 - V2) < 1e-2)]   #OK


#' save out
attr(db_hn_mortality_disaggregated_ado, "timestamp") <- Sys.Date() # set date
fn <- here("disaggregation/output/db_hn_mortality_disaggregated_ado.Rdata")
save(db_hn_mortality_disaggregated_ado, file = fn)
