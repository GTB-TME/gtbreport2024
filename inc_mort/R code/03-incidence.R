...
#' - India extra model for India 2015-2023, stored in ~/input_inc
...
#'    from regional workshops of 2009-2011, stored in ~/inc_mort/input_inc/rules_inc.csv
...
#'    the rules_inc.csv file is superseded by the output from mathematical models or by results of 
...
md.exclude <- c('GEO', 'ETH', 'PRK', 'ZAF','CHN','RUS',"UZB")
...
sel <- est$year %in% 2020:yr & est$iso3 %in% extra.lst & est$iso3 %ni% md.exclude
table(sel)
...
# 2024 
# temporary update SAU higher bound after discussion with NTP, waiting for epi review
# CDR = 10%
# Fixing higher bound to 1.2

rules$hi[rules$iso3=="SAU"]=1.2
...
hic <- c(hic, 'ANT', 'WLF', 'WSM','FSM','PLW','SLB','TON','BTN','PSE','LBN',"LCA","DJI","SLV","CPV","COK","MHL","KIR")
...
# 2024: Back to case notif and std adjustment for ARG, CUB, ECU, GTM, PRY, GRD
#       Since regional model do not well reflect increase in notif in these countries and 
#       estimated incidence < notifications

rmtostd= c("ARG","CUB","ECU","GTM","PRY","GRD")
...
# Set UZB estimates to NA for 2020:2023 due to a change in method, now projected trend
est["UZB",.(iso3,year,inc,inc.sd)]
vale= est$year %in% 2020:2023 & est$iso3=="UZB"
est[vale,inc := NA]
est[vale,inc.sd := NA]
...
#Exlcude India from this list to keep 2020-2022 old estimates
hat.lst=hat.lst[hat.lst!="IND"]

#Exclude RUS and CHINA
hat.lst=hat.lst[hat.lst!="RUS"]
hat.lst=hat.lst[hat.lst!="CHN"]

#Set incidence to projected values for 2020:yr
...
lst <-
  c('FJI', 'TUV', 'BGD', 'BTN', 'LBY', 'SUR', 'DMA','TLS',"VUT")
...
(est['VUT', .(iso3, year, inc, inc.sd)])

### CHN and RUS specific


#' CHN 2023
#' 
#' After communication with Country Office, Under reporting in 2023 was similar to 2020
#' Country estimated incidence at 53.4 in 2022 vs 52.5 for GTB, adjust this difference in 2023 estimates for being consistent

est["CHN",.(year,inc,newinc,newinc/inc,inc.sd)]

sel <- est$iso3=='CHN' & est$year==2023
ratechn20=est$newinc[est$iso3=='CHN' & est$year==2020]/est$inc[est$iso3=='CHN' & est$year==2020]
adjfactchn=53.4/est$inc[est$iso3=='CHN' & est$year==2022]
est$inc[sel] <- est$newinc[sel]/ratechn20/adjfactchn
est$inc.sd[sel] <- est$inc.sd[est$iso3=="CHN"& est$year==2020]
...
sel <- est$iso3=='RUS' & est$year==2023
...
#
# Ethiopia : incidence increase same rate as notification increase in 2021:2023
#

est["ETH", .(iso3, year, inc, inc.sd, c.newinc, newinc)]

#rate of increase in 2023/2022
sel23=est$iso3=='ETH' & est$year==2023
sel22=est$iso3=='ETH' & est$year==2022
sel21=est$iso3=='ETH' & est$year==2021

rate.eth23=est$newinc[sel23]/est$newinc[sel22]
rate.eth22=est$newinc[sel22]/est$newinc[sel21]

est$inc[sel22]=est$inc[sel21]*rate.eth22
est$inc.sd[sel22]=est$inc.sd[sel21]*rate.eth22

est$inc[sel23]=est$inc[sel22]*rate.eth23
est$inc.sd[sel23]=est$inc.sd[sel22]*rate.eth23
...
#'
#' # Incorporate the new estimates for India for 2023
#'
#'
...
for (i in 2011:yr) {
  est[iso3=="IND" & year==i, inc:=india$val[india$measure=="inc"& india$year==i]]
  est[iso3=="IND" & year==i, inc.lo:=india$val[india$measure=="inclo" & india$year==i]]
  est[iso3=="IND" & year==i, inc.hi:=india$val[india$measure=="inchi" & india$year==i]]
  est[iso3=="IND" & year==i, inc.sd:=(inc.hi-inc.lo)/3.92]
}


sel <- est$year %in% 2011:yr & est$iso3 =="IND"
est[sel, source.inc := "Country model"]
...
# plot incidence from modeled countries (country specific model)
...
#update extra.lst
extra.lst=unique(est$iso3[est$source.inc=="Regional model"  & est$year==2023]) 
...
### Impute UNAIDS estimate for 2023 since not yet available for 1st round
...
# PAK 2020-2021 most probably too high and unreliable, remove and interpolate
est["PAK",.(year,tbhiv,tbhiv.sd)]
est[iso3=="PAK" & year %in% 2020:2021, tbhiv := NA]
est[iso3=="PAK" & year %in% 2020:2021, tbhiv.sd := NA]
est[iso3=="PAK", tbhiv := na.approx(tbhiv, na.rm = FALSE)]
est[iso3=="PAK", tbhiv.sd := na.approx(tbhiv, na.rm = FALSE)]
est[iso3=="PAK" & year %in% 2020:2021, inc.h := NA]
est[iso3=="PAK" & year %in% 2020:2021, inc.h.sd := NA]
...
sel <- est$year %in% 2020:yr & est$iso3 %in% mdh.lst & est$iso3 %ni% md.exclude
table(sel)
...
sel <- est$year %in% 2020:yr & est$iso3 %in% rmdh.lst & est$iso3 %ni% md.exclude
table(sel)
...
# multiple IRR available
#

# If IRR missing in 2023, impute
est[n.irr > 1 & year==2023 & is.na(irr), .(iso3,year,irr)]
irrmiss.lst=unique(est$iso3[est$n.irr > 1 & est$year==2023 & is.na(est$irr)])

est[n.irr > 1 & iso3 %in% irrmiss.lst, f.irr := mean(irr, na.rm = T) / mean(e.irr, na.rm = T)]
est[n.irr > 1 & iso3 %in% irrmiss.lst, f.irr.sd := mean(irr.sd, na.rm = T) / mean(irr, na.rm = T)]
est[n.irr > 1 & iso3 %in% irrmiss.lst, f.irr := max(f.irr, na.rm = TRUE), by = iso3]
est[n.irr > 1 & iso3 %in% irrmiss.lst, f.irr.sd := max(f.irr.sd, na.rm = TRUE), by = iso3]
est[n.irr > 1 & iso3 %in% irrmiss.lst, irr := e.irr * f.irr]
est[n.irr > 1 & iso3 %in% irrmiss.lst, irr.sd := irr * f.irr.sd]
...
sel2 <- est$iso3=='CHN' & est$year==2017

est[sel, tbhiv.sd := est$tbhiv.sd[sel2]]
...
# Fix Serbia 2020/2021/2022/2023
# Serbia not anymore regional model
# For 2022-2023: use notification and previous CDR
# For 2020/2021: interpolate incidence using <2020 and 2022 value

sel = est$iso3=="SRB" & est$year %in% 2020:yr
est[sel, inc:=NA]
est[sel, inc.sd:=NA]
est[sel, inc.nh:=NA]
...
sel = est$iso3=="SRB" & est$year==2023
...
#inc.nh
sel = est$iso3=="SRB" & est$year %in% 2020:2023
incnh <- with(est[sel], prodXY(inc, (1 - tbhiv), inc.sd, tbhiv.sd))
est[sel, inc.nh := incnh[[1]]]
est[sel, inc.nh.sd := incnh[[2]]]
...
# Fix PAK bounds of HIV, fix sd at 20% (no data)
est["PAK",.(year,tbhiv,tbhiv.sd,inc.h,inc.h.sd,inc.h.lo)]
leonmarchand= est$iso3=="PAK" & est$year %in% 2020:2023
est[leonmarchand, inc.h.sd:=0.2*inc.h]
est[leonmarchand, inc.h.lo:=inc.h-1.96*inc.h.sd]
est[leonmarchand, inc.h.hi:=inc.h+1.96*inc.h.sd]


#Fix CZE 2022

teddyriner=est$iso3=="CZE" & est$year==2022
inch <- with(est[teddyriner], prodXY(inc, tbhiv, inc.sd, tbhiv.sd))
est[teddyriner, inc.h := inch[[1]]]
est[teddyriner, inc.h.sd := inch[[2]]]
...
(inc.low.lst.15=unique(est[iso3 %ni% hic & iso3 %ni% extra.lst & iso3 %ni% md.lst & iso3!="RUS"  & year==yr & inc<=1.10*newinc,iso3]))
...
fixbound.md= (est$iso3 %in% md.lst & est$iso3 != "IND") & est$year %in% 2020:2023
est$inc.lo[fixbound.md] <- est$inc.md.lo[fixbound.md]
est$inc.hi[fixbound.md] <- est$inc.md.hi[fixbound.md]

fixbound.rmd= (est$iso3 %in% extra.lst) & est$year %in% 2020:2023
...
#bounds for UZB
sel= est$iso3=="UZB" & est$year %in% 2020:yr
...
p <- qplot(year,
  ...
  geom_line(
    aes(year, inc),
    data = subset(old, iso3 %in% md.lst  & year >= 2010),
    colour = I('red'),
    linetype = I(2)
  ) +
  facet_wrap(~ iso3, scales = 'free_y') + xlab('') + ylab('Incidence rate per 100k/yr')
...
### Methods labeling
# est[source.inc=="Country model", source.inc := "Country-specific dynamic model"]
# est[source.inc=="Regional model", source.inc := "Region-specific dynamic model"]
# est[source.inc=="Case notifications, expert opinion", source.inc := "Case notifications and expert opinion"]
# est[source.inc=="Case notifications, standard adjustment", source.inc := "Case notifications with standard adjustment"]
# est[source.inc=="Case notifications, Standard adjustment", source.inc := "Case notifications with standard adjustment"]
...
dta$meth.inc19 <- plyr::revalue(dta$var, c("Country model"="Country-specific dynamic model", 
...
method1=dta[,.(iso3,meth.inc19)]

dta <-
  est[year == yr, .(iso3,source.inc)]
dta$var <- dta$source.inc

dta$meth.inc23 <- plyr::revalue(dta$var, c("Country model"="Country-specific dynamic model", 
                                    "Regional model"="Region-specific dynamic model", 
                                    "Case notifications, standard adjustment"="Notifications and standard adjustment",
                                    "Case notifications, Standard adjustment"="Notifications and standard adjustment",
                                    "Case notifications,\nExpert opinion"="Notifications and expert opinion",
...
method2=dta[,.(iso3,meth.inc23)]


est=merge(est,method1,by="iso3")
est=merge(est,method2,by="iso3")

est$source.inc.old=est$source.inc
est$source.inc=est$meth.inc19
est[meth.inc23=="Country-specific dynamic model" & year %in% 2020:yr, source.inc:=paste0(meth.inc19," combined with ",meth.inc23," in 2020-2023")]
est[meth.inc23=="Region-specific dynamic model" & year %in% 2020:yr, source.inc:=paste0(meth.inc19," combined with ",meth.inc23," in 2020-2023")]

# Fixes
est[iso3=="IND" & year %in% 2000:2019, source.inc:="Prevalence survey and country model"]
est[iso3=="IND" & year %in% 2020:yr, source.inc:="Prevalence survey and country model combined with Country-specific dynamic model in 2020-2023"]

rmtostd= c("ARG","CUB","ECU","GTM","PRY","GRD")
est[iso3 %in% rmtostd & year %in% 2020:2022, source.inc:="Notifications and standard adjustment combined with Region-specific dynamic model in 2020-2022"]
