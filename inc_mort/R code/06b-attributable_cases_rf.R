...
#' 
#' Update 2024:
#'  - under nutrition prevalence was updated to not use anymore undernourishment prevalence and PAF+attributable cases
#'    are estimated in previous script
#'  - Keep the previous estimate using undernourishment
#'  - Update of the RR of undernutrition as per 2024 systematic review
#'  
...
### Merge new attributable cases based on under nutrition and not undernourishment
...
att2=merge(att,malnutpaf,by=c("iso3","sex"))
...
for (i in list(att3, att4, att5, att6, att7,att8)) setnames(i, c('iso3', 'sex', 'lo', 'hi', 'risk.factor', 'best'))
att3[, risk.factor := 'hiv']
att4[, risk.factor := 'diabetes']
att5[, risk.factor := 'alcohol']
att6[, risk.factor := 'smoking']
att7[, risk.factor := 'undernutrition old']
att8[, risk.factor := 'undernutrition']

rf <-
  rbind(att3, att4, att5, att6, att7,att8)
rf[, measure := 'inc']
rf[, unit := 'num']
setkey(rf, iso3)
